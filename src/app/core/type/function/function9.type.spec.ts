 ...
 *
 *    ng test --include src/app/core/type/function/function9.type.spec.ts
 */
describe('isFFunction9', () => {

  it('when no function is provided then false is returned', () => {
    expect(isFFunction9()).toBe(false);
    expect(isFFunction9(null)).toBe(false);
    expect(isFFunction9(12)).toBe(false);
    expect(isFFunction9({})).toBe(false);
  });


  it('when a function that does not match is provided then false is returned', () => {
    expect(isFFunction9((t1: string) => t1)).toBe(false);
    expect(isFFunction9((t1: string, t2: string) => t1 + t2)).toBe(false);
    expect(isFFunction9((t1: string, t2: string, t3: string) => t1 + t2 + t3)).toBe(false);
    expect(isFFunction9((t1: string, t2: string, t3: string, t4: string) => t1 + t2 + t3 + t4)).toBe(false);
    expect(isFFunction9((t1: string, t2: string, t3: string, t4: string, t5: string) => t1 + t2 + t3 + t4 + t5)).toBe(false);
    expect(isFFunction9((t1: string, t2: string, t3: string, t4: string, t5: string, t6: string) => t1 + t2 + t3 + t4 + t5 + t6)).toBe(false);
    expect(isFFunction9((t1: string, t2: string, t3: string, t4: string, t5: string, t6: string, t7: string) => t1 + t2 + t3 + t4 + t5 + t6 + t7)).toBe(false);
    expect(isFFunction9((t1: string, t2: string, t3: string, t4: string, t5: string, t6: string, t7: string, t8: string) => t1 + t2 + t3 + t4 + t5 + t6 + t7 + t8)).toBe(false);
  });


  it('when a function that matches is provided then true is returned', () => {
    expect(isFFunction9((t1: string, t2: string, t3: string, t4: string, t5: string, t6: string, t7: string, t8: string, t9: string) => {})).toBe(true);
    expect(isFFunction9((t1: string, t2: string, t3: string, t4: string, t5: string, t6: string, t7: string, t8: string, t9: string) => t1 + t2 + t3 + t4 + t5 + t6 + t7 + t8 + t9)).toBe(true);
  });
...
describe('Function9', () => {
  ...
  describe('isFunction', () => {
    ...
    it('when no function is provided then false is returned', () => {
      expect(Function9.isFunction()).toBe(false);
      expect(Function9.isFunction(null)).toBe(false);
      expect(Function9.isFunction('')).toBe(false);
      expect(Function9.isFunction(12)).toBe(false);
      expect(Function9.isFunction({})).toBe(false);
      expect(Function9.isFunction({ apply: (s: string, n1: number, n2: number) => s.length + n1 + n2 })).toBe(false);
    });
    ...
    it('when provided function is different than Function9 then false is returned', () => {
      ...
      expect(Function9.isFunction(stringLengthPlusNumbers)).toBe(false);
    });


    it('when a Function9 is provided then true is returned', () => {
      expect(Function9.isFunction(stringLengthPlusNumbersFunction)).toBe(true);
    });
  ...
  describe('of', () => {
    ...
    it('when a raw function equivalent to FFunction9 is provided then a valid Function9 is returned', () => {
      ...
      expect(Function9.isFunction(func)).toBe(true);
