...
/**
 * Arrow function that accepts two arguments and produces a result.
 *
 * @typeParam <T1>
 ...
 *         `false` otherwise
 */
export function isFFunction2<T1, T2, R>(input?: unknown): input is FFunction2<T1, T2, R> {
...
export class Function2<T1, T2, R> {
   ...
   *         `false` otherwise
   */
  static isFunction = <T1, T2, R>(input?: unknown): input is Function2<T1, T2, R> =>
