 ...
 *
 *    ng test --include src/app/core/type/function/function3.type.spec.ts
 */
describe('isFFunction3', () => {

  it('when no function is provided then false is returned', () => {
    expect(isFFunction3()).toBe(false);
    expect(isFFunction3(null)).toBe(false);
    expect(isFFunction3(12)).toBe(false);
    expect(isFFunction3({})).toBe(false);
  });


  it('when a function that does not match is provided then false is returned', () => {
    expect(isFFunction3((t1: string) => t1)).toBe(false);
    expect(isFFunction3((t1: string, t2: string) => t1 + t2)).toBe(false);
  });


  it('when a function that matches is provided then true is returned', () => {
    expect(isFFunction3((t1: string, t2: string, t3: string) => {})).toBe(true);
    expect(isFFunction3((t1: string, t2: string, t3: string) => t1 + t2 + t3)).toBe(true);
  });
...
describe('Function3', () => {
  ...
  describe('isFunction', () => {
    ...
    it('when no function is provided then false is returned', () => {
      expect(Function3.isFunction()).toBe(false);
      expect(Function3.isFunction(null)).toBe(false);
      expect(Function3.isFunction('')).toBe(false);
      expect(Function3.isFunction(12)).toBe(false);
      expect(Function3.isFunction({})).toBe(false);
      expect(Function3.isFunction({ apply: (s: string, n1: number, n2: number) => s.length + n1 + n2 })).toBe(false);
    });
    ...
    it('when provided function is different than Function3 then false is returned', () => {
      ...
      expect(Function3.isFunction(stringLengthPlusNumber)).toBe(false);
    });


    it('when a Function3 is provided then true is returned', () => {
      expect(Function3.isFunction(stringLengthPlusNumbersFunction)).toBe(true);
    });
  ...
  describe('of', () => {
    ...
    it('when a raw function equivalent to FFunction3 is provided then a valid Function3 is returned', () => {
      ...
      expect(Function3.isFunction(func)).toBe(true);
