 ...
 *
 *    ng test --include src/app/core/type/function/function6.type.spec.ts
 */
describe('isFFunction6', () => {

  it('when no function is provided then false is returned', () => {
    expect(isFFunction6()).toBe(false);
    expect(isFFunction6(null)).toBe(false);
    expect(isFFunction6(12)).toBe(false);
    expect(isFFunction6({})).toBe(false);
  });


  it('when a function that does not match is provided then false is returned', () => {
    expect(isFFunction6((t1: string) => t1)).toBe(false);
    expect(isFFunction6((t1: string, t2: string) => t1 + t2)).toBe(false);
    expect(isFFunction6((t1: string, t2: string, t3: string) => t1 + t2 + t3)).toBe(false);
    expect(isFFunction6((t1: string, t2: string, t3: string, t4: string) => t1 + t2 + t3 + t4)).toBe(false);
    expect(isFFunction6((t1: string, t2: string, t3: string, t4: string, t5: string) => t1 + t2 + t3 + t4 + t5)).toBe(false);
  });


  it('when a function that matches is provided then true is returned', () => {
    expect(isFFunction6((t1: string, t2: string, t3: string, t4: string, t5: string, t6: string) => {})).toBe(true);
    expect(isFFunction6((t1: string, t2: string, t3: string, t4: string, t5: string, t6: string) => t1 + t2 + t3 + t4 + t5 + t6)).toBe(true);
  });
...
describe('Function6', () => {
  ...
  describe('isFunction', () => {
    ...
    it('when no function is provided then false is returned', () => {
      expect(Function6.isFunction()).toBe(false);
      expect(Function6.isFunction(null)).toBe(false);
      expect(Function6.isFunction('')).toBe(false);
      expect(Function6.isFunction(12)).toBe(false);
      expect(Function6.isFunction({})).toBe(false);
      expect(Function6.isFunction({ apply: (s: string, n1: number, n2: number) => s.length + n1 + n2 })).toBe(false);
    });
    ...
    it('when provided function is different than Function6 then false is returned', () => {
      ...
      expect(Function6.isFunction(stringLengthPlusNumbers)).toBe(false);
    });


    it('when a Function6 is provided then true is returned', () => {
      expect(Function6.isFunction(stringLengthPlusNumbersFunction)).toBe(true);
    });
  ...
  describe('of', () => {
    ...
    it('when a raw function equivalent to FFunction6 is provided then a valid Function6 is returned', () => {
      ...
      expect(Function6.isFunction(func)).toBe(true);
