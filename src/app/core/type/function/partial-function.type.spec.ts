 ...
 *
 *    ng test --include src/app/core/type/function/partial-function.type.spec.ts
 */
describe('PartialFunction', () => {
  ...
  describe('identity', () => {
    ...
    it('when an input parameter is used then such parameter is returned', () => {
      ...
      expect(identityPartialFunction.isDefinedAt('12')).toBe(true);
      expect(identityPartialFunction.isDefinedAt('abc')).toBe(true);
  ...
  describe('isPartialFunction', () => {
    ...
    it('when no partial function is provided then false is returned', () => {
      expect(PartialFunction.isPartialFunction()).toBe(false);
      expect(PartialFunction.isPartialFunction(null)).toBe(false);
      expect(PartialFunction.isPartialFunction('')).toBe(false);
      expect(PartialFunction.isPartialFunction(12)).toBe(false);
      expect(PartialFunction.isPartialFunction({})).toBe(false);
      expect(PartialFunction.isPartialFunction({ apply: (n: number) => n*2 })).toBe(false);
    });
    ...
    it('when a partial function is provided then true is returned', () => {
      ...
      expect(PartialFunction.isPartialFunction(multiply2ForEven)).toBe(true);

      expect(PartialFunction.isPartialFunction(PartialFunction.identity())).toBe(true);
    });
  ...
  describe('of', () => {
    ...
    it('when null or undefined verifier is given then alwaysTrue Predicate1 will be used', () => {
      ...
      expect(PartialFunction.isPartialFunction(undefinedVerifierPF)).toBe(true);
      expect(undefinedVerifierPF.isDefinedAt(3)).toBe(true);
      expect(undefinedVerifierPF.apply(3)).toEqual(6);

      expect(PartialFunction.isPartialFunction(nullVerifierPF)).toBe(true);
      expect(nullVerifierPF.isDefinedAt(2)).toBe(true);
    ...
    it('when verifier and mapper lambdas are provided then a valid PartialFunction is returned', () => {
      ...
      expect(PartialFunction.isPartialFunction(partialFunction)).toBe(true);
      expect(partialFunction.isDefinedAt(2)).toBe(true);
      expect(partialFunction.isDefinedAt(3)).toBe(false);
  ...
  describe('of2', () => {
    ...
    it('when null or undefined verifier is given then alwaysTrue Predicate2 will be used', () => {
      ...
      expect(PartialFunction.isPartialFunction(undefinedVerifierPF)).toBe(true);
      expect(undefinedVerifierPF.isDefinedAt([2, 3])).toBe(true);
      expect(undefinedVerifierPF.apply([2, 3])).toEqual(6);

      expect(PartialFunction.isPartialFunction(nullVerifierPF)).toBe(true);
      expect(nullVerifierPF.isDefinedAt([5, 7])).toBe(true);
    ...
    it('when verifier and mapper lambdas are provided then a valid PartialFunction is returned', () => {
      ...
      expect(PartialFunction.isPartialFunction(partialFunction)).toBe(true);
      expect(partialFunction.isDefinedAt([2, 4])).toBe(true);
      expect(partialFunction.isDefinedAt([5, 7])).toBe(false);
  ...
  describe('andThen', () => {
    ...
    it('when a FFunction1 is provided then it will be applied after current PartialFunction', () => {
      ...
      expect(stringLengthIfLongerThan3AndThenMultiply2.isDefinedAt('0')).toBe(false);
      expect(stringLengthIfLongerThan3AndThenMultiply2.isDefinedAt('abcd')).toBe(true);
    ...
    it('when a PartialFunction is provided then it will be applied after current one', () => {
      ...
      expect(stringLengthIfLongerThan3AndThenMultiply2IfLowerThan10.isDefinedAt('0')).toBe(false);
      expect(stringLengthIfLongerThan3AndThenMultiply2IfLowerThan10.isDefinedAt('0123456789')).toBe(false);
      expect(stringLengthIfLongerThan3AndThenMultiply2IfLowerThan10.isDefinedAt('abcd')).toBe(true);
  ...
  describe('compose', () => {
    ...
    it('when a FFunction1 is provided then it will be applied before current PartialFunction', () => {
      ...
      expect(addV2AsSuffixComposeStringLengthIfLongerThan3.isDefinedAt('0')).toBe(false);
      expect(addV2AsSuffixComposeStringLengthIfLongerThan3.isDefinedAt('abcd')).toBe(true);
    ...
    it('when a PartialFunction is provided then it will be applied before current one', () => {
      ...
      expect(toStringIfIsEvenComposeStringLengthIfLongerThan3.isDefinedAt(1)).toBe(false);
      expect(toStringIfIsEvenComposeStringLengthIfLongerThan3.isDefinedAt(10)).toBe(false);
      expect(toStringIfIsEvenComposeStringLengthIfLongerThan3.isDefinedAt(1000)).toBe(true);
  ...
  describe('isDefinedAt', () => {
    ...
    it('when input does not belong to domain of PartialFunction then false is returned', () => {
      ...
      expect(partialFunction.isDefinedAt(1)).toBe(false);
      expect(partialFunction.isDefinedAt(5)).toBe(false);
    });
    ...
    it('when input belongs to domain of PartialFunction then true is returned', () => {
      ...
      expect(partialFunction.isDefinedAt(10)).toBe(true);
      expect(partialFunction.isDefinedAt(12)).toBe(true);
    });
  ...
  describe('lift', () => {
    ...
    it('when input does not belong to domain of PartialFunction then Function1 returns an empty Optional', () => {
      ...
      expect(function1.apply(1).isPresent()).toBe(false);
      expect(function1.apply(5).isPresent()).toBe(false);
    });
    ...
    it('when input belongs to domain of PartialFunction then Function1 returns a not empty Optional', () => {
      ...
      expect(function1.apply(10).isPresent()).toBe(true);
      expect(function1.apply(12).isPresent()).toBe(true);
    });
  ...
  describe('orElse', () => {
    ...
    it('when input belongs to domain of current PartialFunction then current will applied', () => {
      ...
      expect(partialFunction.isDefinedAt(22)).toBe(true);
      expect(defaultPartialFunction.isDefinedAt(22)).toBe(false);
      expect(orElsePartialFunction.isDefinedAt(22)).toBe(true);

      expect(partialFunction.isDefinedAt(30)).toBe(true);
      expect(defaultPartialFunction.isDefinedAt(30)).toBe(false);
      expect(orElsePartialFunction.isDefinedAt(30)).toBe(true);
    ...
    it('when input belongs to domain of defaultPartialFunction then defaultPartialFunction will applied', () => {
      ...
      expect(partialFunction.isDefinedAt(11)).toBe(false);
      expect(defaultPartialFunction.isDefinedAt(11)).toBe(true);
      expect(orElsePartialFunction.isDefinedAt(11)).toBe(true);

      expect(partialFunction.isDefinedAt(15)).toBe(false);
      expect(defaultPartialFunction.isDefinedAt(15)).toBe(true);
      expect(orElsePartialFunction.isDefinedAt(15)).toBe(true);
