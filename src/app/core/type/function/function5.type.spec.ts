 ...
 *
 *    ng test --include src/app/core/type/function/function5.type.spec.ts
 */
describe('isFFunction5', () => {

  it('when no function is provided then false is returned', () => {
    expect(isFFunction5()).toBe(false);
    expect(isFFunction5(null)).toBe(false);
    expect(isFFunction5(12)).toBe(false);
    expect(isFFunction5({})).toBe(false);
  });


  it('when a function that does not match is provided then false is returned', () => {
    expect(isFFunction5((t1: string) => t1)).toBe(false);
    expect(isFFunction5((t1: string, t2: string) => t1 + t2)).toBe(false);
    expect(isFFunction5((t1: string, t2: string, t3: string) => t1 + t2 + t3)).toBe(false);
    expect(isFFunction5((t1: string, t2: string, t3: string, t4: string) => t1 + t2 + t3 + t4)).toBe(false);
  });


  it('when a function that matches is provided then true is returned', () => {
    expect(isFFunction5((t1: string, t2: string, t3: string, t4: string, t5: string) => {})).toBe(true);
    expect(isFFunction5((t1: string, t2: string, t3: string, t4: string, t5: string) => t1 + t2 + t3 + t4 + t5)).toBe(true);
  });
...
describe('Function5', () => {
  ...
  describe('isFunction', () => {
    ...
    it('when no function is provided then false is returned', () => {
      expect(Function5.isFunction()).toBe(false);
      expect(Function5.isFunction(null)).toBe(false);
      expect(Function5.isFunction('')).toBe(false);
      expect(Function5.isFunction(12)).toBe(false);
      expect(Function5.isFunction({})).toBe(false);
      expect(Function5.isFunction({ apply: (s: string, n1: number, n2: number) => s.length + n1 + n2 })).toBe(false);
    });
    ...
    it('when provided function is different than Function5 then false is returned', () => {
      ...
      expect(Function5.isFunction(stringLengthPlusNumbers)).toBe(false);
    });


    it('when a Function5 is provided then true is returned', () => {
      expect(Function5.isFunction(stringLengthPlusNumbersFunction)).toBe(true);
    });
  ...
  describe('of', () => {
    ...
    it('when a raw function equivalent to FFunction5 is provided then a valid Function5 is returned', () => {
      ...
      expect(Function5.isFunction(func)).toBe(true);
