...
import { FFunction1, Function1, Function2, isFFunction1 } from '@app-core/type/function';
import { IllegalArgumentError } from '@app-core/error';

/**
 * To invoke only this test:
 *
 *    ng test --include src/app/core/type/function/function1.type.spec.ts
 */
describe('isFFunction1', () => {

  it('when no function is provided then false is returned', () => {
    expect(isFFunction1()).toBe(false);
    expect(isFFunction1(null)).toBe(false);
    expect(isFFunction1(12)).toBe(false);
    expect(isFFunction1({})).toBe(false);
  });


  it('when a function that does not match is provided then false is returned', () => {
    expect(isFFunction1((t1: number, t2: number) => t1 + t2)).toBe(false);
    expect(isFFunction1((t1: number, t2: number, t3: number) => t1 + t2 + t3)).toBe(false);
  });


  it('when a function that matches is provided then true is returned', () => {
    expect(isFFunction1((t1: number) => {})).toBe(true);
    expect(isFFunction1((t1: number) => t1 + 1)).toBe(true);
  });
...
describe('Function1', () => {
  ...
  describe('isFunction', () => {
    ...
    it('when no function is provided then false is returned', () => {
      expect(Function1.isFunction()).toBe(false);
      expect(Function1.isFunction(null)).toBe(false);
      expect(Function1.isFunction('')).toBe(false);
      expect(Function1.isFunction(12)).toBe(false);
      expect(Function1.isFunction({})).toBe(false);
      expect(Function1.isFunction({ apply: (n: number) => n*2 })).toBe(false);
    });
    ...
    it('when provided function is different than Function1 then false is returned', () => {
      ...
      expect(Function1.isFunction(stringLengthPlusNumber)).toBe(false);
    });


    it('when a Function1 is provided then true is returned', () => {
      expect(Function1.isFunction(stringLengthFunction)).toBe(true);
    });
  ...
  describe('of', () => {
    ...
    it('when a raw function equivalent to FFunction1 is provided then a valid Function1 is returned', () => {
      ...
      expect(Function1.isFunction(func)).toBe(true);
