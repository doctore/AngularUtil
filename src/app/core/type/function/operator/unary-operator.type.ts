import { Function1 } from '@app-core/type/function';
 ...
 *         `false` otherwise
 */
export function isFUnaryOperator<T>(input?: unknown): input is FUnaryOperator<T> {
...
export class UnaryOperator<T> extends Function1<T, T> {
   ...
   *         `false` otherwise
   */
  static isUnaryOperator = <T>(input?: unknown): input is UnaryOperator<T> =>
