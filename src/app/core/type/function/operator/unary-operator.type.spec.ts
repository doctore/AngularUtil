import { FUnaryOperator, isFUnaryOperator, UnaryOperator } from '@app-core/type/function/operator';
import {
  FFunction1,
  Function1,
  Function2,
 ...
 *
 *    ng test --include src/app/core/type/function/operator/unary-operator.type.spec.ts
 */
describe('isFUnaryOperator', () => {

  it('when no function is provided then false is returned', () => {
    expect(isFUnaryOperator()).toBe(false);
    expect(isFUnaryOperator(null)).toBe(false);
    expect(isFUnaryOperator(12)).toBe(false);
    expect(isFUnaryOperator({})).toBe(false);
  });


  it('when a function that does not match is provided then false is returned', () => {
    expect(isFUnaryOperator((t1: number, t2: number) => t1 + t2)).toBe(false);
    expect(isFUnaryOperator((t1: number, t2: number, t3: number) => t1 + t2 + t3)).toBe(false);
  });


  it('when a function that matches is provided then true is returned', () => {
    expect(isFUnaryOperator((t1: number) => {})).toBe(true);
    expect(isFUnaryOperator((t1: number) => t1 + 1)).toBe(true);
  });
...
describe('UnaryOperator', () => {
  ...
  describe('isUnaryOperator', () => {
    ...
    it('when no function is provided then false is returned', () => {
      expect(UnaryOperator.isUnaryOperator()).toBe(false);
      expect(UnaryOperator.isUnaryOperator(null)).toBe(false);
      expect(UnaryOperator.isUnaryOperator('')).toBe(false);
      expect(UnaryOperator.isUnaryOperator(12)).toBe(false);
      expect(UnaryOperator.isUnaryOperator({})).toBe(false);
      expect(UnaryOperator.isUnaryOperator({ apply: (n: number) => n*2 })).toBe(false);
    });
    ...
    it('when provided function is different than UnaryOperator then false is returned', () => {
      ...
      expect(UnaryOperator.isUnaryOperator(stringLengthPlusNumber)).toBe(false);
    });


    it('when a UnaryOperator is provided then true is returned', () => {
      expect(UnaryOperator.isUnaryOperator(stringV2UnaryOperator)).toBe(true);
    });
  ...
  describe('of', () => {
    ...
    it('when a raw function equivalent to FUnaryOperator is provided then a valid UnaryOperator is returned', () => {
      ...
      expect(UnaryOperator.isUnaryOperator(func)).toBe(true);
