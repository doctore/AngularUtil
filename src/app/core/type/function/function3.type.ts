...
/**
 * Arrow function that accepts three arguments and produces a result.
 *
 * @typeParam <T1>
 ...
 *         `false` otherwise
 */
export function isFFunction3<T1, T2, T3, R>(input?: unknown): input is FFunction3<T1, T2, T3, R> {
...
export class Function3<T1, T2, T3, R> {
   ...
   *         `false` otherwise
   */
  static isFunction = <T1, T2, T3, R>(input?: unknown): input is Function3<T1, T2, T3, R> =>
   ...
   *
   * @return {@link FFunction3}
   */
  getMapper = (): FFunction3<T1, T2, T3, R> =>
    this.mapper;
