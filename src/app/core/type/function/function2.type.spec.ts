 ...
 *
 *    ng test --include src/app/core/type/function/function2.type.spec.ts
 */
describe('isFFunction2', () => {

  it('when no function is provided then false is returned', () => {
    expect(isFFunction2()).toBe(false);
    expect(isFFunction2(null)).toBe(false);
    expect(isFFunction2(12)).toBe(false);
    expect(isFFunction2({})).toBe(false);
  });


  it('when a function that does not match is provided then false is returned', () => {
    expect(isFFunction2((t1: number) => t1 + 1)).toBe(false);
    expect(isFFunction2((t1: string, t2: string, t3: string) => t1 + t2 + t3)).toBe(false);
  });


  it('when a function that matches is provided then true is returned', () => {
    expect(isFFunction2((t1: string, t2: string) => t1 + t2)).toBe(true);
    expect(isFFunction2((t1: string, t2: string) => {})).toBe(true);
  });
...
describe('Function2', () => {
  ...
  describe('isFunction', () => {
    ...
    it('when no function is provided then false is returned', () => {
      expect(Function2.isFunction()).toBe(false);
      expect(Function2.isFunction(null)).toBe(false);
      expect(Function2.isFunction('')).toBe(false);
      expect(Function2.isFunction(12)).toBe(false);
      expect(Function2.isFunction({})).toBe(false);
      expect(Function2.isFunction({ apply: (s: string, n: number) => s.length + n })).toBe(false);
    });
    ...
    it('when provided function is different than Function2 then false is returned', () => {
      ...
      expect(Function2.isFunction(stringLength)).toBe(false);
    });


    it('when a Function2 is provided then true is returned', () => {
      expect(Function2.isFunction(stringLengthPlusNumbersFunction)).toBe(true);
    });
  ...
  describe('of', () => {
    ...
    it('when a raw function equivalent to FFunction2 is provided then a valid Function2 is returned', () => {
      ...
      expect(Function2.isFunction(func)).toBe(true);
