...
import { Consumer3, Consumer4, FConsumer4, isFConsumer4 } from '@app-core/type/consumer';
import { IllegalArgumentError } from '@app-core/error';

/**
 * To invoke only this test:
 *
 *    ng test --include src/app/core/type/consumer/consumer4.type.spec.ts
 */
describe('isFConsumer4', () => {

  it('when no function is provided then false is returned', () => {
    expect(isFConsumer4()).toBe(false);
    expect(isFConsumer4(null)).toBe(false);
    expect(isFConsumer4(12)).toBe(false);
    expect(isFConsumer4({})).toBe(false);
  });
  ...
  it('when a function that does not match is provided then false is returned', () => {
    expect(isFConsumer4(() => {})).toBe(false);
    expect(isFConsumer4((t1: number) => { t1 += 1; } )).toBe(false);
    expect(isFConsumer4((t1: number, t2: number) => t1 + t2)).toBe(false);
    expect(isFConsumer4((t1: number, t2: number, t3: number) => t1 + t2 + t3)).toBe(false);
  });


  it('when a function that matches is provided then true is returned', () => {
    expect(isFConsumer4((t1: string, t2: string, t3: string, t4: string) => {})).toBe(true);
    expect(isFConsumer4((t1: number, t2: number, t3: number, t4: number) => t1 + t2 + t3 + t4)).toBe(true);
  });
...
describe('Consumer4', () => {
  ...
  describe('isConsumer', () => {
    ...
    it('when no consumer is provided then false is returned', () => {
      expect(Consumer4.isConsumer()).toBe(false);
      expect(Consumer4.isConsumer(null)).toBe(false);
      expect(Consumer4.isConsumer('')).toBe(false);
      expect(Consumer4.isConsumer(12)).toBe(false);
      expect(Consumer4.isConsumer({})).toBe(false);
    });
    ...
    it('when provided consumer is different than Consumer4 then false is returned', () => {
      ...
      expect(Consumer4.isConsumer(consumer)).toBe(false);
    });
    ...
    it('when a Consumer4 is provided then true is returned', () => {
      ...
      expect(Consumer4.isConsumer(consumer)).toBe(true);
    });
