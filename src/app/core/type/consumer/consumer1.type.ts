 ...
 *         `false` otherwise
 */
export function isFConsumer1<T>(input?: unknown): input is FConsumer1<T> {
...
export class Consumer1<T> {
   ...
   *         `false` otherwise
   */
  static isConsumer = <T>(input?: unknown): input is Consumer1<T> =>
    ObjectUtil.nonNullOrUndefined(input) &&
    undefined !== (input as Consumer1<T>).andThen &&
    undefined !== (input as Consumer1<T>).apply &&
    undefined !== (input as Consumer1<T>).getAction &&
    isFConsumer1((input as Consumer1<T>).getAction());
   ...
   *
   * @return {@link FConsumer1}
   */
  getAction = (): FConsumer1<T> =>
    this.action;
