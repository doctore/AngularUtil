...
import { Consumer0, Consumer1, FConsumer1, isFConsumer1 } from '@app-core/type/consumer';
import { IllegalArgumentError } from '@app-core/error';

/**
 * To invoke only this test:
 *
 *    ng test --include src/app/core/type/consumer/consumer1.type.spec.ts
 */
describe('isFConsumer1', () => {

  it('when no function is provided then false is returned', () => {
    expect(isFConsumer1()).toBe(false);
    expect(isFConsumer1(null)).toBe(false);
    expect(isFConsumer1(12)).toBe(false);
    expect(isFConsumer1({})).toBe(false);
  });
  ...
  it('when a function that does not match is provided then false is returned', () => {
    expect(isFConsumer1(() => {})).toBe(false);
    expect(isFConsumer1((t1: number, t2: number) => t1 + t2)).toBe(false);
    expect(isFConsumer1((t1: number, t2: number, t3: number) => t1 + t2 + t3)).toBe(false);
  });


  it('when a function that matches is provided then true is returned', () => {
    expect(isFConsumer1((t1: string) => {} )).toBe(true);
    expect(isFConsumer1((t1: number) => { t1 += 1; } )).toBe(true);
  });
...
describe('Consumer1', () => {
  ...
  describe('isConsumer', () => {
    ...
    it('when no consumer is provided then false is returned', () => {
      expect(Consumer1.isConsumer()).toBe(false);
      expect(Consumer1.isConsumer(null)).toBe(false);
      expect(Consumer1.isConsumer('')).toBe(false);
      expect(Consumer1.isConsumer(12)).toBe(false);
      expect(Consumer1.isConsumer({})).toBe(false);
    });


    it('when provided consumer is different than Consumer1 then false is returned', () => {
      const consumer: Consumer0 =
        Consumer0.of(() => { let a = 1;  a += 2; });

      expect(Consumer1.isConsumer(consumer)).toBe(false);
    });


    it('when a Consumer1 is provided then true is returned', () => {
      const consumer: Consumer1<NullableOrUndefined<number>> =
        Consumer1.of((n: NullableOrUndefined<number>) => { n! += 2; });

      expect(Consumer1.isConsumer(consumer)).toBe(true);
    });
