...
import { Consumer2, Consumer3, FConsumer2, isFConsumer2 } from '@app-core/type/consumer';
import { IllegalArgumentError } from '@app-core/error';

/**
 * To invoke only this test:
 *
 *    ng test --include src/app/core/type/consumer/consumer2.type.spec.ts
 */
describe('isFConsumer2', () => {

  it('when no function is provided then false is returned', () => {
    expect(isFConsumer2()).toBe(false);
    expect(isFConsumer2(null)).toBe(false);
    expect(isFConsumer2(12)).toBe(false);
    expect(isFConsumer2({})).toBe(false);
  });
  ...
  it('when a function that does not match is provided then false is returned', () => {
    expect(isFConsumer2(() => {})).toBe(false);
    expect(isFConsumer2((t1: number) => { t1 += 1; } )).toBe(false);
    expect(isFConsumer2((t1: number, t2: number, t3: number) => t1 + t2 + t3)).toBe(false);
  });


  it('when a function that matches is provided then true is returned', () => {
    expect(isFConsumer2((t1: string, t2: string) => {} )).toBe(true);
    expect(isFConsumer2((t1: number, t2: number) => t1 + t2)).toBe(true);
  });
...
describe('Consumer2', () => {
  ...
  describe('isConsumer', () => {
    ...
    it('when no function is provided then false is returned', () => {
      expect(Consumer2.isConsumer()).toBe(false);
      expect(Consumer2.isConsumer(null)).toBe(false);
      expect(Consumer2.isConsumer('')).toBe(false);
      expect(Consumer2.isConsumer(12)).toBe(false);
      expect(Consumer2.isConsumer({})).toBe(false);
    });
    ...
    it('when provided consumer is different than Consumer2 then false is returned', () => {
      ...
      expect(Consumer2.isConsumer(consumer)).toBe(false);
    });
    ...
    it('when a Consumer2 is provided then true is returned', () => {
      ...
      expect(Consumer2.isConsumer(consumer)).toBe(true);
    });
  ...
  describe('apply', () => {
    ...
    it('when a Consumer2 is provided then the defined operation is performed based on provided arguments', () => {
      let externalInt = 10;
      let externalString = 'abc';
