...
import { Consumer4, Consumer5, FConsumer5, isFConsumer5 } from '@app-core/type/consumer';
import { IllegalArgumentError } from '@app-core/error';

/**
 * To invoke only this test:
 *
 *    ng test --include src/app/core/type/consumer/consumer5.type.spec.ts
 */
describe('isFConsumer5', () => {

  it('when no function is provided then false is returned', () => {
    expect(isFConsumer5()).toBe(false);
    expect(isFConsumer5(null)).toBe(false);
    expect(isFConsumer5(12)).toBe(false);
    expect(isFConsumer5({})).toBe(false);
  });
  ...
  it('when a function that does not match is provided then false is returned', () => {
    expect(isFConsumer5(() => {})).toBe(false);
    expect(isFConsumer5((t1: number) => { t1 += 1; } )).toBe(false);
    expect(isFConsumer5((t1: number, t2: number) => t1 + t2)).toBe(false);
    expect(isFConsumer5((t1: number, t2: number, t3: number) => t1 + t2 + t3)).toBe(false);
    expect(isFConsumer5((t1: number, t2: number, t3: number, t4: number) => t1 + t2 + t3 + t4)).toBe(false);
  });


  it('when a function that matches is provided then true is returned', () => {
    expect(isFConsumer5((t1: string, t2: string, t3: string, t4: string, t5: string) => {})).toBe(true);
    expect(isFConsumer5((t1: number, t2: number, t3: number, t4: number, t5: number) => t1 + t2 + t3 + t4 + t5)).toBe(true);
  });
...
describe('Consumer5', () => {
  ...
  describe('isConsumer', () => {
    ...
    it('when no consumer is provided then false is returned', () => {
      expect(Consumer5.isConsumer()).toBe(false);
      expect(Consumer5.isConsumer(null)).toBe(false);
      expect(Consumer5.isConsumer('')).toBe(false);
      expect(Consumer5.isConsumer(12)).toBe(false);
      expect(Consumer5.isConsumer({})).toBe(false);
    });
    ...
    it('when provided consumer is different than Consumer5 then false is returned', () => {
      ...
      expect(Consumer5.isConsumer(consumer)).toBe(false);
    });
    ...
    it('when a Consumer5 is provided then true is returned', () => {
      ...
      expect(Consumer5.isConsumer(consumer)).toBe(true);
    });
