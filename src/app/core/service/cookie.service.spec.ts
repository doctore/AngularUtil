...
import { CookieService } from '@app-core/service';
import { CookieOptions } from '@app-core/model';

/**
 * To launch only this test:
 *
 *    ng test --include src/app/core/service/cookie.service.spec.ts
 */
describe('CookieService', () => {

  let mockDocumentCookieGet: ReturnType<typeof vi.spyOn>;
  let mockDocumentCookieSet: ReturnType<typeof vi.spyOn>;
  ...
  beforeEach(() => {
    ...
    mockDocumentCookieGet = vi.spyOn(mockDocument, 'cookie', 'get')
      .mockImplementation(originalCookieDescriptor.get!);

    mockDocumentCookieSet = vi.spyOn(mockDocument, 'cookie', 'set')
      .mockImplementation(originalCookieDescriptor.set!);
  ...
  afterEach(() => {
    mockDocumentCookieGet.mockClear();
    mockDocumentCookieSet.mockClear();
  });
  ...
  describe('Platform browser', () => {
    ...
    describe('check', () => {
      ...
      it('when the Cookie does not exist then false is returned', () => {
        mockDocumentCookieGet.mockReturnValue('foo=123');
      ...
      it('when the Cookie exists then only checks for names not for values', () => {
        mockDocumentCookieGet.mockReturnValue('c1=123; c2=456');
      ...
      it('when the name of the Cookie is encoded then searching by decoded name returns true', () => {
        mockDocumentCookieGet.mockReturnValue('%3B%2C%2F%3F%3A%40%26%3D%2B%24=exists;');
    ...
    describe('get', () => {
      ...
      it('when the Cookie exists and their values are encoded then their decoded values are returned', () => {
        ...
        mockDocumentCookieGet.mockReturnValue(cookieString);
      ...
      it('when the Cookie exists but decoding its value fails then original value is returned', () => {
        mockDocumentCookieGet.mockReturnValue('foo=%E0%A4%A');
    ...
    describe('getAll', () => {
      ...
      it('when there are no Cookies then empty array is returned', () => {
        mockDocumentCookieGet.mockReturnValue('');
    ...
    describe('getOptional', () => {
      ...
      it('when the Cookie does not exist then empty Optional is returned', () => {
        ...
        expect(service.getOptional('NotFound').isPresent()).toBe(false);
      });
    ...
    describe('set', () => {
      ...
      it('when name/value with special characters are provided then encoded name/value with default options are used for the new Cookie', () => {
        expect(service.set('-H@llö_ Wörld-', '-H@llö_ Wörld-')).toBe(true);
        expect(service.set(';,/?:@&=+$', ';,/?:@&=+$')).toBe(true);
        expect(service.set('-H@llö_ Wörld-', '-H@llö_ Wörld-')).toBe(true);
        expect(service.set('$uper^TEST(123)', '$uper^TEST(123)')).toBe(true);
        expect(service.set('F()!!()/bar', 'F()!!()/bar')).toBe(true);
        expect(service.set('*F.)/o(o*', '*F.)/o(o*')).toBe(true);
        expect(service.set('-O_o{1,2}', '-O_o{1,2}')).toBe(true);
        expect(service.set('B?ar|Fo+o', 'B?ar|Fo+o')).toBe(true);
        expect(service.set('Hello=World;', 'Hello=World;')).toBe(true);
        expect(service.set('[foo-_*.]bar', '[foo-_*.]bar')).toBe(true);
  ...
  describe('Platform server', () => {
    ...
    describe('delete', () => {
      ...
      it('then always return false', () => {
        expect(service.delete('foo')).toBe(false);
    ...
    describe('getOptional', () => {
      ...
      it('then always return empty Optional', () => {
        ...
        expect(service.getOptional('foo').isPresent()).toBe(false);
      });
    ...
    describe('set', () => {
      ...
      it('then always return false', () => {
        expect(service.set('foo', '123')).toBe(false);
        expect(mockDocumentCookieSet).not.toHaveBeenCalled();
      });
