...
export class ArrayUtil {
  ...
  /**
   *    Returns from the given `sourceArray` the first position of the element that verifies the provided `filterPredicate`,
  ...
  static filterFirstIndex = <T>(sourceArray: NullableOrUndefined<T[]>,
    ...
    for (let i = 0; i < sourceArray!.length; i++) {
      if (finalFilterPredicate.apply(sourceArray![i])) {
        return i;
      }
    }
    return -1;
  ...
  static groupMapMultiKey = <T, K, V>(sourceArray: NullableOrUndefined<T[]>,
    ...
    const result: Map<K, V[]> = new Map<K, V[]>();
    if (!this.isEmpty(sourceArray)) {
      const finalValueMapper = Function1.of(
        valueMapper
      );
      const finalDiscriminatorKey = Function1.of(
        discriminatorKey
   ...
   *         `false` otherwise.
   */
  static isEmpty = (arrayToVerify?: Nullable<unknown[]>): boolean =>
    !arrayToVerify || 0 == arrayToVerify.length
  ...
  static sort = <T>(sourceArray: NullableOrUndefined<T[]>,
    ...
    const clonedSourceArray = _.cloneDeep(
      sourceArray
    );
    return comparator
  ...
  static uniqueByProperties<T, K extends keyof T>(sourceArray: NullableOrUndefined<T[]>,
    ...
    const finalMergeValueFunction = BinaryOperator.isBinaryOperator(mergeValueFunctionOrKeepFirstFound)
                                    || isFBinaryOperator(mergeValueFunctionOrKeepFirstFound)
