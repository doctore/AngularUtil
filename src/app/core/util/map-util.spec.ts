...
import { FBinaryOperator } from '@app-core/type/function/operator';
import { FPredicate2, Predicate2 } from '@app-core/type/predicate';
...
describe('MapUtil', () => {
  ...
  describe('collectFirst', () => {
    ...
    it('when given sourceMap has no elements and partialFunction is provided then empty Optional is returned', () => {
      ...
      expect(MapUtil.collectFirst(null, keyAndValueLengthForOdd).isPresent()).toBe(false);
      expect(MapUtil.collectFirst(undefined, keyAndValueLengthForOdd).isPresent()).toBe(false);
      expect(MapUtil.collectFirst(emptyMap, keyAndValueLengthForOdd).isPresent()).toBe(false);
    });
    ...
    it('when given sourceMap has elements and partialFunction is defined but no one element matches with its domain then an empty Optional is returned', () => {
      ...
      expect(MapUtil.collectFirst(sourceMap, keyAndValuePlus1ForOddPP).isPresent()).toBe(false);
    });
  ...
  describe('filter', () => {
    ...
    it('when given sourceMap has elements and filterPredicate is provided then filtered Map is returned', () => {
  ...
  describe('filterFirst', () => {
    ...
    it('when given sourceMap is undefined, null or is an empty Map then undefined is returned', () => {
      ...
      expect(MapUtil.filterFirst(undefined, areKeyValueEvenPredicate)).toBe(undefined);
      expect(MapUtil.filterFirst(emptyMap, areKeyValueEvenPredicate)).toBe(undefined);
    });


    it('when given sourceMap is not empty but filterPredicate is null or undefined then undefined is returned', () => {
      const sourceMap = new Map<number, number>();
      sourceMap.set(11, 19);

      expect(MapUtil.filterFirst(sourceMap, undefined)).toBe(undefined);
      expect(MapUtil.filterFirst(sourceMap, null)).toBe(undefined);
    });


    it('when there is no element that matches provided filter then undefined is returned', () => {
      const u1 = new User(1, 'user1');
      const u2 = new User(2, 'user2');
      const u3 = new User(3, 'user3');
    ...
    it('when there is an element that matches provided filter then expected element is returned', () => {
      const r1 = { id: 1, name: 'role1' } as Role;
      const r2 = { id: 2, name: 'role2' } as Role;
      const r3 = { id: 3, name: 'role3' } as Role;
      ...
      const result = MapUtil.filterFirst(sourceMap, isKeyAndRoleIdOddPredicate);
  ...
  describe('findValue', () => {
    ...
    it('when given sourceMap is undefined, null or is an empty Map and item is provided then undefined is returned', () => {
      ...
      expect(MapUtil.findValue(undefined, 1)).toBe(undefined);
      expect(MapUtil.findValue(emptyMap, 1)).toBe(undefined);
    });
    ...
    it('when given sourceMap is undefined, null or is an empty array and item and equalsFunction are provided then undefined is returned', () => {
      ...
      expect(MapUtil.findValue(null, 1, isEvenRaw)).toBe(undefined);
      expect(MapUtil.findValue(emptyMap, 1, isEvenRaw)).toBe(undefined);
    });
    ...
    it('when given sourceMap is not empty but item is null or undefined then undefined is returned', () => {
      ...
      // @ts-ignore
      expect(MapUtil.findValue(sourceMap, undefined)).toBe(undefined);

      // @ts-ignore
      expect(MapUtil.findValue(sourceMap, null)).toBe(undefined);
    });
    ...
    it('when given sourceMap is not empty, item is provided but there is no value that matches then undefined is returned', () => {
      ...
      const roleToSearch = { id: 4, name: 'role4' } as Role;

      expect(MapUtil.findValue(sourceMap, roleToSearch, areRolesEqualsByIdRaw)).toBe(undefined);
      expect(MapUtil.findValue(sourceMap, roleToSearch, areAllRolePropertiesEqualsFPredicate)).toBe(undefined);
      expect(MapUtil.findValue(sourceMap, roleToSearch, areRolesEqualsPredicate)).toBe(undefined);
    });
    ...
    it('when given sourceMap is not empty, item is provided and there is a value that matches then expected one is returned', () => {
      ...
      const u2SameId = new User(2, 'user2 v2');
      const u2Cloned = new User(2, 'user2');

      expect(MapUtil.findValue(sourceMap, u2)).toBe(u2);
      expect(MapUtil.findValue(sourceMap, u3, null)).toBe(u3);
      expect(MapUtil.findValue(sourceMap, u3, areAllUserPropertiesEqualsFPredicate)).toBe(u3);
      expect(MapUtil.findValue(sourceMap, u2SameId, areUsersEqualsByIdRaw)).toBe(u2);
      expect(MapUtil.findValue(sourceMap, u2Cloned, areAllUserPropertiesEqualsFPredicate)).toBe(u2);
      expect(MapUtil.findValue(sourceMap, u3, areUsersEqualsPredicate)).toBe(u3);
    });
  ...
  describe('findValueOptional', () => {
    ...
    it('when given sourceMap is undefined, null or is an empty Map and item is provided then empty Optional is returned', () => {
      ...
      expect(MapUtil.findValueOptional(undefined, 1).isPresent()).toBe(false);
      expect(MapUtil.findValueOptional(emptyMap, 1).isPresent()).toBe(false);
    });
    ...
    it('when given sourceMap is undefined, null or is an empty array and item and equalsFunction are provided then empty Optional is returned', () => {
      ...
      expect(MapUtil.findValueOptional(null, 1, isEvenRaw).isPresent()).toBe(false);
      expect(MapUtil.findValueOptional(emptyMap, 1, isEvenRaw).isPresent()).toBe(false);
    });
    ...
    it('when given sourceMap is not empty but item is null or undefined then undefined is returned', () => {
      ...
      // @ts-ignore
      expect(MapUtil.findValueOptional(sourceMap, undefined).isPresent()).toBe(false);

      // @ts-ignore
      expect(MapUtil.findValueOptional(sourceMap, null).isPresent()).toBe(false);
    });
    ...
    it('when given sourceMap is not empty, item is provided but there is no value that matches then empty Optional is returned', () => {
      ...
      expect(MapUtil.findValueOptional(sourceMap, roleToSearch, areRolesEqualsByIdRaw).isPresent()).toBe(false);
      expect(MapUtil.findValueOptional(sourceMap, roleToSearch, areAllRolePropertiesEqualsFPredicate).isPresent()).toBe(false);
      expect(MapUtil.findValueOptional(sourceMap, roleToSearch, areRolesEqualsPredicate).isPresent()).toBe(false);
    });
    ...
    it('when given sourceMap is not empty, item is provided and there is a value that matches then Optional with expected one is returned', () => {
      ...
      expect(MapUtil.findValueOptional(sourceMap, u2).isPresent()).toBe(true);
      expect(MapUtil.findValueOptional(sourceMap, u2).get()).toEqual(u2);

      expect(MapUtil.findValueOptional(sourceMap, u3, null).isPresent()).toBe(true);
      expect(MapUtil.findValueOptional(sourceMap, u3, null).get()).toEqual(u3);

      expect(MapUtil.findValueOptional(sourceMap, u3, areAllUserPropertiesEqualsFPredicate).isPresent()).toBe(true);
      expect(MapUtil.findValueOptional(sourceMap, u3, areAllUserPropertiesEqualsFPredicate).get()).toEqual(u3);

      expect(MapUtil.findValueOptional(sourceMap, u2SameId, areUsersEqualsByIdRaw).isPresent()).toBe(true);
      expect(MapUtil.findValueOptional(sourceMap, u2SameId, areUsersEqualsByIdRaw).get()).toEqual(u2);

      expect(MapUtil.findValueOptional(sourceMap, u2Cloned, areAllUserPropertiesEqualsFPredicate).isPresent()).toBe(true);
      expect(MapUtil.findValueOptional(sourceMap, u2Cloned, areAllUserPropertiesEqualsFPredicate).get()).toEqual(u2);

      expect(MapUtil.findValueOptional(sourceMap, u3, areUsersEqualsPredicate).isPresent()).toBe(true);
      expect(MapUtil.findValueOptional(sourceMap, u3, areUsersEqualsPredicate).get()).toEqual(u3);
    });
  ...
  describe('isMap', () => {
    ...
    it('when given input is null or undefined then false will be returned', () => {
      ...
      expect(MapUtil.isMap()).toEqual(expectedResult);
      expect(MapUtil.isMap(undefined)).toEqual(expectedResult);
      expect(MapUtil.isMap(null)).toEqual(expectedResult);
    });


    it('when given input is not a Map then false will be returned', () => {
      const user = new User(1, 'user1');

      const expectedResult = false;

      expect(MapUtil.isMap(12)).toEqual(expectedResult);
      expect(MapUtil.isMap("abc")).toEqual(expectedResult);
      expect(MapUtil.isMap({})).toEqual(expectedResult);
      expect(MapUtil.isMap(user)).toEqual(expectedResult);
    });


    it('when given input is a Map then true will be returned', () => {
      const map = new Map<number, string>();
      map.set(1, "a");

      const expectedResult = true;

      expect(MapUtil.isMap(new Map<string, boolean>)).toEqual(expectedResult);
      expect(MapUtil.isMap(map)).toEqual(expectedResult);
    });
  ...
  describe('max', () => {
    ...
    it('when given sourceMap has no elements and comparator is not provided then undefined is returned', () => {
      ...
      // @ts-ignore
      expect(MapUtil.max(null, null)).toBe(undefined);
      // @ts-ignore
      expect(MapUtil.max(undefined, undefined)).toBe(undefined);
      // @ts-ignore
      expect(MapUtil.max(emptyMap, null)).toBe(undefined);
      // @ts-ignore
      expect(MapUtil.max(emptyMap, undefined)).toBe(undefined);
    });
    ...
    it('when given sourceMap has no elements and comparator is provided then undefined is returned', () => {
      ...
      expect(MapUtil.max(null, compareKeys)).toBe(undefined);
      expect(MapUtil.max(undefined, compareKeys)).toBe(undefined);
      expect(MapUtil.max(emptyMap, compareKeys)).toBe(undefined);
    });
  ...
  describe('maxOptional', () => {
    ...
    it('when given sourceMap has no elements and comparator is not provided then empty Optional is returned', () => {
      ...
      // @ts-ignore
      expect(MapUtil.maxOptional(null, null).isPresent()).toBe(false);
      // @ts-ignore
      expect(MapUtil.maxOptional(undefined, undefined).isPresent()).toBe(false);
      // @ts-ignore
      expect(MapUtil.maxOptional(emptyMap, null).isPresent()).toBe(false);
      // @ts-ignore
      expect(MapUtil.maxOptional(emptyMap, undefined).isPresent()).toBe(false);
    });
    ...
    it('when given sourceMap has no elements and comparator is provided then empty Optional is returned', () => {
      ...
      expect(MapUtil.maxOptional(null, compareKeys).isPresent()).toBe(false);
      expect(MapUtil.maxOptional(undefined, compareKeys).isPresent()).toBe(false);
      expect(MapUtil.maxOptional(emptyMap, compareKeys).isPresent()).toBe(false);
    });
    ...
    it('when given sourceMap is not empty and comparator is valid then its largest element is returned', () => {
      ...
      expect(resultKeys.isPresent()).toBe(true);
      expect(resultKeys.get()).toEqual([11, 'k']);

      expect(resultElements.isPresent()).toBe(true);
  ...
  describe('min', () => {
    ...
    it('when given sourceMap has no elements and comparator is not provided then undefined is returned', () => {
      ...
      // @ts-ignore
      expect(MapUtil.min(null, null)).toBe(undefined);
      // @ts-ignore
      expect(MapUtil.min(undefined, undefined)).toBe(undefined);
      // @ts-ignore
      expect(MapUtil.min(emptyMap, null)).toBe(undefined);
      // @ts-ignore
      expect(MapUtil.min(emptyMap, undefined)).toBe(undefined);
    });
    ...
    it('when given sourceMap has no elements and comparator is provided then undefined is returned', () => {
      ...
      expect(MapUtil.min(null, compareKeys)).toBe(undefined);
      expect(MapUtil.min(undefined, compareKeys)).toBe(undefined);
      expect(MapUtil.min(emptyMap, compareKeys)).toBe(undefined);
    });
  ...
  describe('minOptional', () => {
    ...
    it('when given sourceMap has no elements and comparator is not provided then empty Optional is returned', () => {
      ...
      // @ts-ignore
      expect(MapUtil.minOptional(null, null).isPresent()).toBe(false);
      // @ts-ignore
      expect(MapUtil.minOptional(undefined, undefined).isPresent()).toBe(false);
      // @ts-ignore
      expect(MapUtil.minOptional(emptyMap, null).isPresent()).toBe(false);
      // @ts-ignore
      expect(MapUtil.minOptional(emptyMap, undefined).isPresent()).toBe(false);
    });
    ...
    it('when given sourceMap has no elements and comparator is provided then empty Optional is returned', () => {
      ...
      expect(MapUtil.minOptional(null, compareKeys).isPresent()).toBe(false);
      expect(MapUtil.minOptional(undefined, compareKeys).isPresent()).toBe(false);
      expect(MapUtil.minOptional(emptyMap, compareKeys).isPresent()).toBe(false);
    });
  ...
  describe('reduce', () => {
    ...
    it('when given sourceMap is null, undefined or empty then undefined is returned', () => {
      ...
      expect(MapUtil.reduce(null, accumulator)).toBe(undefined);
      expect(MapUtil.reduce(undefined, accumulator)).toBe(undefined);
      expect(MapUtil.reduce(emptyMap, accumulator)).toBe(undefined);
    });
  ...
  describe('setIfAbsent', () => {
    ...
    it('when given sourceMap is empty then key and value is always added', () => {
      ...
      expect(MapUtil.setIfAbsent(emptyMap, key, value)).toBe(undefined);
      expect(emptyMap.size).toEqual(1);
      expect(emptyMap.has(key)).toBe(true);
    });
    ...
    it('when given sourceMap does not contain the key then new value is added and undefined is returned', () => {
      ...
      expect(MapUtil.setIfAbsent(map, key, value)).toBe(undefined);
  ...
  describe('sort', () => {
    ...
    it('using interfaces, when given sourceMap is not empty and comparator is valid then the sorted map using comparator is returned', () => {
      const r1 = { id: 1, name: 'role1' } as Role;
      const r2 = { id: 2, name: 'role2' } as Role;
      const r3 = { id: 3, name: 'role3' } as Role;
    ...
    it('using classes, when given sourceMap is not empty and comparator is valid then the sorted map using comparator is returned', () => {
      const u1 = new User(1, 'user1');
      const u2 = new User(2, 'user2');
      const u3 = new User(3, 'user3');
