...
/**
 * Helper functions to manage {@link Map}.
 */
export class MapUtil {
   ...
   * <pre>
   *    find(                                                                  Result:
   *      [(1, 'Hi'), (2, 'Hello'), (3, 'World'), (5, 'Mundo')],                (3, 'World')
   *      (k: number, v: string) => 1 == k % 2 && 2 > v.length()
   *    )
   * </pre>
   *
   ...
   */
  static filterFirst = <K, V>(sourceMap: NullableOrUndefined<Map<K, V>>,
                              filterPredicate: NullableOrUndefined<TPredicate2<K, V>>): OrUndefined<[K, V]> => {
    if (!this.isEmpty(sourceMap) &&
   ...
   * <pre>
   *    findValue(                                                                                        Result:
   *      [(1, {id: 1, name: 'user1'}), (2, {id: 2, name: 'user2'}), (3, {id: 3, name: 'user3'})],         {id: 2, name: 'user2'}
   *      {id: 2, name: 'user2 v2'},
   ...
   *    {@link Map} to search
   * @param valueToSearch
   *    Value to compare and search in `sourceMap`
   * @param equalsFunction
   *    {@link TPredicate2} used to compare `valueToSearch` with the current value in `sourceMap`. If it is `null` or
   *    `undefined` then {@link _.isEqual} will be used.
   *
   * @return `undefined` if `sourceMap` has no elements, `valueToSearch` is `null` or `undefined`, or no one value verifies
   *         provided `equalsFunction`.
   *         Otherwise, the first value that verifies `equalsFunction` using `valueToSearch` and the current value
   *         of `sourceMap` as parameters.
   */
  static findValue = <K, V>(sourceMap: NullableOrUndefined<Map<K, V>>,
                            valueToSearch: V,
                            equalsFunction?: NullableOrUndefined<TPredicate2<V, V>>): OrUndefined<V> => {
    if (!this.isEmpty(sourceMap) &&
        ObjectUtil.nonNullOrUndefined(valueToSearch)) {

      const finalEqualsFunction = equalsFunction
        ? Predicate2.of(equalsFunction)
        : Predicate2.of(
            (a: V, b: V) => _.isEqual(a, b)
          );

      for (const value of sourceMap!.values()) {
        if (finalEqualsFunction.apply(value, valueToSearch)) {
          return value;
        }
      }
    }
  ...
  /**
   *    Searching in the values of the given `sourceMap`, returns an {@link Optional} containing the first value of the
   * given `sourceMap` equals to `valueToSearch`, based on the provided {@link TPredicate2} `equalsFunction`.
   * {@link Optional#empty} otherwise.
   *
   * <pre>
   *    findValueOptional(                                                                                Result:
   *      [(1, {id: 1, name: 'user1'}), (2, {id: 2, name: 'user2'}), (3, {id: 3, name: 'user3'})],         {id: 2, name: 'user2'}
   *      {id: 2, name: 'user2 v2'},
   *      (u1: User, u2: User) => u1.id == u2.id
   * </pre>
   ...
   *    Value to compare and search in `sourceMap`
   * @param equalsFunction
   *    {@link TPredicate2} used to compare `value` with the current value in `sourceMap`. If it is `null` or `undefined`
   *    then {@link _.isEqual} will be used.
   *
   * @return {@link Optional#empty} if `sourceMap` has no elements, `valueToSearch` is `null` or `undefined`, or no one
   *         value verifies provided `equalsFunction`.
   *         Otherwise, {@link Optional} containing the first value that verifies `equalsFunction` using `valueToSearch`
   *         and the current value of `sourceMap` as parameters.
   */
  static findValueOptional = <K, V>(sourceMap: NullableOrUndefined<Map<K, V>>,
                                    valueToSearch: V,
                                    equalsFunction?: NullableOrUndefined<TPredicate2<V, V>>): Optional<V> =>
    Optional.ofNullable(
      this.findValue(
        sourceMap,
        valueToSearch,
  ...
  /**
   * Verifies if the given `input` is classified as {@link Map} object.
   *
   * @param input
   *    Object to verify
   *
  ...
  static removeAll<K, V>(sourceMap: NullableOrUndefined<Map<K, V>>,
    ...
    const finalAreEqualsComparison = this.getFinalAreEqualsComparison(
      areEqualsComparison
    );
    const result = new Map<K, V>();
