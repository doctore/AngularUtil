...
import { FFunction1, FFunction2, Function1, PartialFunction } from '@app-core/type/function';
import { NullableOrUndefined } from '@app-core/type';
import { FPredicate1 } from '@app-core/type/predicate';
import { FUnaryOperator } from '@app-core/type/function/operator';
...
describe('StringUtil', () => {
  ...
  describe('containsIgnoreCase', () => {
    ...
    it('when given sourceString is null or undefined then false is returned', () => {
      expect(StringUtil.containsIgnoreCase(null, null)).toBe(false);
      expect(StringUtil.containsIgnoreCase(null, undefined)).toBe(false);
      expect(StringUtil.containsIgnoreCase(null, '')).toBe(false);
      expect(StringUtil.containsIgnoreCase(null, 'ab')).toBe(false);

      expect(StringUtil.containsIgnoreCase(undefined, null)).toBe(false);
      expect(StringUtil.containsIgnoreCase(undefined, undefined)).toBe(false);
      expect(StringUtil.containsIgnoreCase(undefined, '')).toBe(false);
      expect(StringUtil.containsIgnoreCase(undefined, 'ab')).toBe(false);
    });
    ...
    it('when given stringToSearch is null or undefined then false is returned', () => {
      ...
      expect(StringUtil.containsIgnoreCase(undefined, null)).toBe(false);
      expect(StringUtil.containsIgnoreCase('', null)).toBe(false);
      expect(StringUtil.containsIgnoreCase('ab', null)).toBe(false);

      expect(StringUtil.containsIgnoreCase(null, undefined)).toBe(false);
      expect(StringUtil.containsIgnoreCase(undefined, undefined)).toBe(false);
      expect(StringUtil.containsIgnoreCase('', undefined)).toBe(false);
      expect(StringUtil.containsIgnoreCase('ab', undefined)).toBe(false);
    });


    it('when given sourceString contains characters but stringToSearch does not appear then false is returned', () => {
      expect(StringUtil.containsIgnoreCase('', 'abd')).toBe(false);
      expect(StringUtil.containsIgnoreCase('abcab', 'abd')).toBe(false);
      expect(StringUtil.containsIgnoreCase('abcab', 'x')).toBe(false);
      expect(StringUtil.containsIgnoreCase('abcab', 'xx')).toBe(false);
    });


    it('when given sourceString contains stringToSearch then true is returned', () => {
      expect(StringUtil.containsIgnoreCase('', '')).toBe(true);
      expect(StringUtil.containsIgnoreCase('ab', '')).toBe(true);

      expect(StringUtil.containsIgnoreCase('abcd', 'bc')).toBe(true);
      expect(StringUtil.containsIgnoreCase('abcd', 'D')).toBe(true);
      expect(StringUtil.containsIgnoreCase('ABcD', 'bC')).toBe(true);
      expect(StringUtil.containsIgnoreCase('abcd', 'ABCD')).toBe(true);
    });
  ...
  describe('isBlank', () => {
    ...
    it('when given sourceString is not empty and no whitespace then false is returned', () => {
      expect(StringUtil.isBlank('112')).toBe(false);
      expect(StringUtil.isBlank('  a  ')).toBe(false);
    });
  ...
  describe('isEmpty', () => {
    ...
    it('when given sourceString is not empty then false is returned', () => {
      ...
      expect(StringUtil.isEmpty('112')).toBe(false);
      expect(StringUtil.isEmpty('  a  ')).toBe(false);
    });
